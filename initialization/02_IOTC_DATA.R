...
#NC_RAW  = NC_raw(years = 1950:2021, factorize_results = FALSE)[IS_IOTC_SPECIES == TRUE]
#write.csv(NC_RAW, "../inputs/data/IOTC_NC_RAW.csv", row.names = FALSE)
NC_RAW = fread("../inputs/data/IOTC_NC_RAW.csv")

# CONSOLIDATE ####
NC_RAW[SPECIES_CODE %in% c("BXQ"), `:=` (SPECIES_CODE = "BIL", SPECIES = "Marlins,sailfishes,etc. nei")]
NC_RAW[SPECIES_CODE %in% c("TUS", "AG06", "AG10", "AG35"), `:=` (SPECIES_CODE = "TUN", SPECIES = "Tunas nei", SPECIES_CATEGORY_CODE = "TROPICAL")]
